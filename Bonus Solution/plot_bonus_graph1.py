...
def plot_improved_task1_accuracy():
    ...
    print("Running experiment to generate Graph 1 (Improved Online EWC)...")
    ...
    for task_id in range(num_tasks):
        ...
        for epoch in range(epochs_per_task):
            ...
            acc = evaluate(sgd_model, task1_test_loader, device)
            sgd_task1_accs.append(acc * 100)
    ...
    for task_id in range(num_tasks):
        ...
        for epoch in range(epochs_per_task):
            ...
            acc = evaluate(ewc_model, task1_test_loader, device)
            ewc_task1_accs.append(acc * 100)
    ...
    plt.plot(epochs, sgd_task1_accs, marker='o', label='Standard SGD (ImprovedMLP)')
    plt.plot(epochs, ewc_task1_accs, marker='s', label='Online EWC (ImprovedMLP)')
    ...
    plt.title('Performance on Task 1 while sequentially learning new tasks (Improved)')
    ...
    save_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'bonus_results_graph.png'))
    plt.savefig(save_path)
    print(f"Graph 1 saved to: {save_path}")

if __name__ == '__main__':
    plot_improved_task1_accuracy()
